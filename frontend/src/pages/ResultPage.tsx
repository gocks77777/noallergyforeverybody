...
export default function ResultPage() {
  ...
  if (!state?.result) {
    return (
      <div className="p-6 text-center space-y-4">
        <p className="text-gray-500">No analysis result.</p>
        <button onClick={() => navigate('/')} className="text-primary-600 font-medium">
          Go back and scan food
        </button>
      </div>
    )
  }
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-white shadow">
        ...
        <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/70 to-transparent p-4">
          ...
          <p className="text-sm text-white/80">
            {result.top3[0]?.score && `Confidence: ${(result.top3[0].score * 100).toFixed(1)}%`}
          </p>
        </div>
      </div>
      ...
      {hasDanger ? (
        <div className="bg-danger-50 border border-danger-200 rounded-xl p-4 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-danger-700 font-bold">Allergen Warning</h3>
          </div>
        ...
        <div className="bg-primary-50 border border-primary-200 rounded-xl p-4 flex items-center gap-2">
          ...
          <p className="text-primary-700 font-medium">No known allergens detected</p>
        </div>
      )}
          ...
          {result.ingredients.map((ing) => (
            <span
              ...
              className={`px-2 py-0.5 text-sm rounded-md ${
                result.allergens.some((a) => ing.includes(a))
      ...
      {/* Top 3 Predictions */}
      <section className="bg-white rounded-xl shadow p-4 space-y-2">
        <h3 className="text-sm font-semibold text-gray-700">Top Predictions</h3>
        {result.top3.map((item, i) => (
          <div key={i} className="flex items-center gap-3">
            <span className="text-xs text-gray-400 w-4">{i + 1}</span>
            <div className="flex-1">
              <div className="flex justify-between text-sm">
                <span className={i === 0 ? 'font-semibold text-primary-700' : 'text-gray-600'}>
                  {item.label}
                </span>
              ...
              <div className="mt-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full ${i === 0 ? 'bg-primary-500' : 'bg-gray-300'}`}
